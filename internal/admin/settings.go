...
func (a *Admin) GetSettingsOIDC(c *fiber.Ctx) error {
	rows, err := a.DB.Query(
		`SELECT id, provider_type, name, issuer_url, client_id, scopes, groups_claim, required_group, enabled, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.ProviderType, &p.Name, &p.IssuerURL, &p.ClientID, &p.Scopes, &p.GroupsClaim, &p.RequiredGroup, &p.Enabled, &p.CreatedAt); err != nil {
...
func (a *Admin) CreateSettingsOIDC(c *fiber.Ctx) error {
	...
	if req.ProviderType == "" || req.Name == "" || req.ClientID == "" || req.ClientSecret == "" || req.IssuerURL == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "provider_type, name, client_id, client_secret, and issuer_url are required"})
	}
	...
	id := uuid.New().String()
	_, err = a.DB.Exec(
		`INSERT INTO oidc_providers (id, provider_type, name, client_id, client_secret, client_secret_salt, issuer_url, scopes, groups_claim, required_group, enabled)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		id, req.ProviderType, req.Name, req.ClientID, encryptedSecret, salt, req.IssuerURL, req.Scopes, req.GroupsClaim, req.RequiredGroup, req.Enabled,
	)
	if err != nil {
...
func (a *Admin) UpdateSettingsOIDC(c *fiber.Ctx) error {
	...
	if req.ProviderType != "" {
		if _, err := a.DB.Exec(`UPDATE oidc_providers SET provider_type = ? WHERE id = ?`, req.ProviderType, id); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to update"})
		}
	}
	if req.Name != "" {
		if _, err := a.DB.Exec(`UPDATE oidc_providers SET name = ? WHERE id = ?`, req.Name, id); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to update"})
		}
	}
	if req.IssuerURL != "" {
	...
	if req.GroupsClaim != "" {
		if _, err := a.DB.Exec(`UPDATE oidc_providers SET groups_claim = ? WHERE id = ?`, req.GroupsClaim, id); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to update"})
		}
	}
	if req.RequiredGroup != nil {
		if _, err := a.DB.Exec(`UPDATE oidc_providers SET required_group = ? WHERE id = ?`, *req.RequiredGroup, id); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "failed to update"})
		}
	}
	if req.Enabled != nil {
	...
	if err := a.loadOIDCProvider(id); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": fmt.Sprintf("provider updated but failed to reinitialize: %v", err),
		})
	}
